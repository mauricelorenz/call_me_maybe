...
def get_next_token(states: States, logits: List[float]) -> List[int]:
    main_state, param_state = states.get_states()
    if main_state == "START":
...
def call_llm(llm: Any, functions_definition: List[FunctionsDefinition],
    ...
    generated = []
    while max_tokens:
        logits = llm.get_logits_from_input_ids(full_prompt_tokens + generated)
        token = get_next_token(states, logits)
